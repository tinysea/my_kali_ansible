...
'''
def get_github_metadata(repo,tag):
    """
    Get the release metadata by tag from Github, returns json.
    """
...
def get_github_releases(repo):
    """
    Get the release metadata from Github, returns json.
    """
...
def get_github_release(repo, regex, output_directory, name=None):
    ...
    try:
        ...
        try_in_older_releases(repo, regex, output_directory, name, releases)

def try_in_older_releases(repo, regex, output_directory, name, releases):
    """
    Get the latest release from Github and download the file that matches the regex.
    Returns: Nothing
    """
    ...
    try:
        ...
        verify_directory_exists(output_directory)
        metadata = get_github_metadata(repo,release)
        url = get_github_release_url(metadata, regex)
        f_bytes = get_http_binary_file(url)
